...
def create_app(config=None, testing=False, app_name="Airflow", server_uri=None, ai_flow_server_uri=None):
    ...
    with flask_app.app_context():
        ...
        init_appbuilder_views(flask_app, server_uri, ai_flow_server_uri)
...
def cached_app(config=None, testing=False):
    ...
    if not app:
        app = create_app(config=config, testing=testing, server_uri=os.getenv('SERVER_URI', None),
                         ai_flow_server_uri=os.getenv('AI_FLOW_SERVER_URI'))
    return app
